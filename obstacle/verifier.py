...
def main(argv):
  ...
  N = vr.ReadInt(2, 100_000)
  vr.ReadEndl()
  A = vr.ReadIntList(N, 0, 100_000)

  assert all(a != 0 for a in A[::2]), 'An odd-indexed element is zero'
  assert all(a == 0 for a in A[1::2]), 'An even-indexed element is not zero'
