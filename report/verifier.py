...
def main(argv):
  ...
  B = vr.ReadString("01", N, N)
  vr.ReadEndl()

  B = list(B)   # make it mutable
  ...
  for _ in range(Q):
    ...
    if t == 1:
      x = vr.ReadInt(1, N)
      assert B[x-1] == '0', "query 1, but B[x] != 0"
      B[x-1] = '1'

    if t == 2:
      x = vr.ReadInt(1, N)
      assert B[x-1] == '1', "query 2, but B[x] != 1"
      B[x-1] = '0'
  ...
  assert scenario3 != 0, "at least one scenario type 3"
  vr.ReadEof()
